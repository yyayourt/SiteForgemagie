...
## ⭐ HIÉRARCHIE DES SOURCES — lire ceci avant tout le reste

**SOURCE PRIMAIRE, retrouvée le 10/09/2026 :**
[`sources/archive/DEVBLOG-ANKAMA-ORIGINAL.md`](sources/archive/DEVBLOG-ANKAMA-ORIGINAL.md)
— texte **intégral et verbatim** du DevBlog Ankama « La nouvelle forgemagie »
(`devblog.dofus.com/fr/billets/61-nouvelle-forgemagie.html`, récupéré sur
web.archive.org, traduction DE identique). C'est la **seule** parole d'Ankama sur le sujet.
**En cas de conflit, elle tranche, sans discussion.**

**SOURCES SECONDAIRES — relais communautaires, parfois FAUTIFS :**
blog Guilde Yin-Yang, blog Alterya, wiki forgemagie.net (ExiTeD), tutoriels
Dofus-Touch, threads JOL… Aucun d'eux n'a jamais cité le DevBlog intégralement ;
tous en dérivent de seconde main. Erreurs avérées, désormais corrigées ici :

| Erreur du corpus secondaire | Vérité du DevBlog |
|---|---|
| ancre « jet parfait » = 34/50/16 (Yin-Yang) | **43/50/7** (Alterya et ExiTeD avaient raison) |
| triplet « 1/22/77 exo avec puits » | **n'existe pas** — invention d'un relais |
| « l'exo, c'est 1 % » (universel) | 1 % est le **plancher** ; la création d'effet monte à **32/50/18** |
| « pas de SN en création d'effet » | le SN existe, jusqu'à 50 % ; il n'est nul **qu'au plancher** |
| un seul « cap 101 » | **deux** plafonds distincts : par effet (101) et par objet (inconnu) |
| — | 3 facteurs jamais cités : **éthéré**, **mono-jet naturel**, **nb d'over/exo** |
| — | « si le bonus a un **jet fixe**, le palier des 80 % ne s'applique pas » |
| — | SN sur objet mono-jet = **rien ne se passe** |
...
Item            : {
                    template_id : int
                    level       : int
                    ethere      : bool            # objet ÉTHÉRÉ -> FM plus difficile   [OBS]
                  ...
                  # dérivé : mono_jet = (len(template) == 1) -> FM plus FACILE       [OBS]
...
* Une ligne **over** = `value > jet_max` avec `jet_max > 0`. Pour un **MALUS**, « overmaxé »
  signifie **amélioré au-delà du meilleur malus naturel** (`value < jet_min`) : c'est la seule
  situation où un malus peut redevenir une source de perte (§5.3). **[OBS]**
* Un **bonus** ne descend jamais sous **0** ; un **malus** ne dépasse jamais le **malus
  maximum naturel**. **[OBS]**
...
⭐ **Recopiés du texte primaire** (`sources/archive/DEVBLOG-ANKAMA-ORIGINAL.md`,
section « Quelques probabilités »). Formulation d'Ankama **verbatim**. **[OBS]**

| # | Situation (formulation Ankama exacte) | SC | SN | EC |
|---|---|---:|---:|---:|
...
**Deux régimes, un seul continuum.** Les ancres 1-2-3 encadrent le **remontage**,
les ancres 4-5 encadrent la **création d'effet**. Rien dans le texte d'Ankama ne
sépare formellement les deux : ce sont les mêmes trois facteurs de difficulté, plus
le fait qu'une ligne non naturelle « augmente encore davantage » la difficulté.
➡️ Le modèle les traite comme **un continuum unique** (un simple `coef` de nature de
ligne), et c'est vérifié : `fm_sim.py` produit 32/50/18 sur une création d'effet
facile et 1/0/99 sur un exo PA/PM/PO, **sans aucun câblage** du type
`if exo: return 0.01`. Voir §3.7 et le test T1.

**Trois règles dures qui en découlent, et qui doivent être des invariants testés :**
```
INV-1   P_SC ≥ 1 %       toujours, dans TOUS les cas.                    [OBS]
INV-2   P_SN ≤ 50 %      « Les chances d'obtenir un succès partiel sont
                           au maximum de 50 % ».                          [OBS]
INV-3   P_SC ≤ 66 %      plafond absolu du succès critique (ancre 1).     [OBS]
```
...
**littéralement** l'ancre 5. **[CODE]**

⚠️ **Ce plancher `p_sc ≤ 1 ⇒ (1, 0, 99)` est un invariant de BORNE, pas un câblage
de l'exo.** C'est lui qui explique le « 1 % » que toute la communauté observe sur
les exos PA/PM/PO : leurs runes pèsent 100/90/51 et la ligne n'est pas naturelle,
donc `a/b` tombe sous 1. Une création d'effet légère sur un objet léger ne tombe
PAS au plancher : elle monte jusqu'à l'ancre 4.

⚠️ **Le triplet « 1/22/77 — exo avec puits » a été SUPPRIMÉ du corpus.** Il ne
figure ni dans l'original FR ni dans la traduction DE : c'est une invention (ou une
déformation) du relais Yin-Yang. Toute calibration faite dessus était corrompue.
Les paramètres `sn_avec_puits` / `puits_donne_sn` ont disparu de `ModelParams`.

## 3.2 Les facteurs de difficulté, par ordre d'importance décroissant **[OBS]**

Liste **complète et verbatim** du DevBlog (§ « Chances de réussites »). Elle est
nettement plus riche que ce que reprenaient les guides.

**Les trois facteurs annoncés, par ordre décroissant d'importance :**
1. **Qualité globale de l'objet** — « plus les jets de l'objet sont proches du
   maximum, plus l'objet a une bonne qualité, et la forgemagie est difficile ».
   Les over et les exos **comptent** dans cette qualité. **MAIS** :
   > « le jet en cours de modification **n'est pas pris en compte** dans le calcul
   > de la qualité, afin de limiter l'impact de ce facteur sur les objets bas-niveau
   > ou avec un seul jet. »

   ➡️ `fm_sim.py` : `pwr_g(exclure=stat_ciblée)` alimente `mStat`
   (`ModelParams.exclure_jet_cible_de_qualite`, défaut **True**).
2. **Qualité du jet modifié** — « il y a un malus supplémentaire lorsque l'on
   dépasse le jet parfait (overmax) ». Le seuil est un **palier** :
   > « la difficulté augmente **brutalement** à 80 % du jet. **Si le bonus a un jet
   > fixe, ce facteur n'est pas pris en compte.** »

   ➡️ La clause « jet fixe » n'était documentée nulle part : un PA, un PM, une PO,
   un dommage fixe… n'ont pas de fourchette, donc pas de palier 80 %.
   `ModelParams.jet_fixe_ignore_palier`, défaut **True**.
3. **Niveau de l'objet** — « la difficulté augmente **faiblement** avec le niveau ».
   ➡️ `ModelParams.coef_niveau_objet` (U32), `a *= (1 − k·niveau/200)`.

**Quatre facteurs supplémentaires, cités dans le même paragraphe et jamais repris :**
* « Si le jet que l'on modifie est un overmax, ou n'est pas un bonus existant
  naturellement sur l'objet, la difficulté augmente encore davantage.
  **Plus l'objet dispose d'overmax / bonus exotiques (en prenant en compte celui en
  cours de modification), plus la difficulté augmente.** »
  ➡️ **Point remarquable** : le jet ciblé est **COMPTÉ** ici alors qu'il est **EXCLU**
  de la qualité globale (facteur 1). Deux traitements différents du même jet dans la
  même formule. `Item.nb_over_exo(stat_ciblée, rune.add)` et
  `ModelParams.penalite_par_over_exo` (`a *= f ** n`).
* « Il est plus difficile de forgemager un jet jusqu'à une valeur élevée **sans
  utiliser des runes puissantes**. » ➡️ c'est la **règle du ×20** (§7.2), et elle est
  donc **[OBS]**, plus seulement **[CONS]**. Facteur `t`.
* « La forgemagie des objets disposant naturellement d'**un seul jet** est plus
  **facile**. » ➡️ `ModelParams.bonus_mono_jet` (> 1).
* « La forgemagie des objets **éthérés** est plus **difficile**. »
  ➡️ `ModelParams.malus_ethere` (< 1), et `Item.ethere`.

**Ce qui reste absent de la liste d'Ankama** : le niveau du métier, le nombre de
lignes en tant que tel, le reliquat/puits. Le poids de la rune n'apparaît qu'à
travers « sans utiliser des runes puissantes ».
...
# --- 0. Cas bloquants — DEUX plafonds distincts (§7.1) ----------------------
si poids_non_naturel(ligne) après tentative > CAP_EFFET (101)  → (0, 0, 1)  # plafond PAR EFFET
si poids_non_naturel(OBJET) après tentative > CAP_OBJET         → (0, 0, 1)  # plafond PAR OBJET,
                                                # valeur INCONNUE -> None = désactivé par défaut
...
si JET FIXE (jet_min == jet_max) et value_apres <= jet_max : f = 0   # « si le bonus a un jet
                                                # fixe, ce facteur n'est pas pris en compte » [OBS]
si f > 1.0                    : c = (1 − (f − 1)/2) / 2       # au-delà du max naturel
sinon si f > SEUIL_DEGRESSIF  : c = 1 − f/2                    # SEUIL_DEGRESSIF = 0.80
sinon                         : c = 1
c = max(0, c) ** EXPOSANT_C

# --- 2. mStat : qualité globale de l'objet ----------------------------------
# ⚠️ « le jet en cours de modification n'est pas pris en compte dans le calcul
#     de la qualité » -> on utilise PWRg PRIVÉ de la ligne visée.          [OBS]
PWRg_qualite = PWRg − poids_pondéré(ligne_visée)
moyenne = PWRmax − (PWRmax − PWRmin)/2
mStat   = min(PLAFOND_MSTAT, moyenne / max(1, PWRg_qualite))  # PLAFOND_MSTAT
...
# --- 5bis. NOUVEAU — les 4 facteurs du DevBlog jamais modélisés ailleurs ----
f_niveau = max(0.05, 1 − COEF_NIVEAU_OBJET * niveau_objet / 200)   # « faiblement »
f_ethere = MALUS_ETHERE      si objet éthéré        sinon 1        # < 1
f_mono   = BONUS_MONO_JET    si 1 seul jet NATUREL  sinon 1        # > 1
n_oe     = nb de lignes over ou exotiques, JET CIBLÉ COMPTÉ (état APRÈS la tentative)
f_oe     = PENALITE_PAR_OVER_EXO ** n_oe                           # <= 1
...
a = taille * coef * mStat * c * x * t * f_niveau * f_ethere * f_mono * f_oe * RATE_FM
...
si p_sc <= 1 :        # PLANCHER OFFICIEL (ancre 5) : 1/0/99, quelle que soit la
                      #   présence d'un puits. Invariant de BORNE, pas câblage de l'exo.
    p_sc, p_sn = 1, 0
sinon :
...
Constantes par défaut, **toutes [SUPP] sauf les clamps 1/50/66 et CAP_EFFET, qui
sont [OBS]** :
...
SEUIL_X20_DUR=20 · PENAL_X20=0.35 · EXPOSANT_X20=2 · CAP_EFFET=101 · CAP_OBJET=None ·
COEF_NIVEAU_OBJET=0 · MALUS_ETHERE=1 · BONUS_MONO_JET=1 · PENALITE_PAR_OVER_EXO=1`
(ces défauts = **StarLoco à l'identique**, les 4 facteurs nouveaux étant neutres ;
pour un simulateur, préférer le préréglage ajusté du §3.7.)

**Note sur le puits et les probabilités.** Le corpus secondaire affirmait qu'avec
un puits l'exo passait de 1/0/99 à 1/22/77. **Ce triplet n'existe pas.** Le DevBlog
dit seulement que la magie résiduelle « absorbera **en partie** les échecs futurs » :
c'est une mécanique de **perte**, pas de **probabilité**. `fm_sim.py` ne fait donc
plus varier le triplet avec le puits ; `ModelParams.puits_absorption` (défaut 1.0)
règle la part de la perte que le puits encaisse, et `puits_augmente_p_sc`
(hypothèse Ancestra, défaut False) reste disponible pour tester l'hypothèse inverse.

## 3.7 RÉSULTAT DE L'AJUSTEMENT SUR LES ANCRES OFFICIELLES (fait, reproductible)

`reference_impl/fm_sim.py` contient les 5 situations du DevBlog codées comme
scénarios (`ancres_devblog()`), un score d'erreur (`score_ancres()`), un rapport
d'écart (`ecarts_ancres()`) et un calibrateur (`calibrate_multistart()`).
Résultats **effectivement obtenus** (`python3 fm_sim.py`) :

| Ancre (cible) | StarLoco brut | **Ancien** préréglage<br>(calibré sur les ancres FAUSSES) | **Nouveau** préréglage |
|---|---|---|---|
| 1 remontage, meilleur cas — 66/34/0 | 66/34/0 ✅ | 66/34/0 ✅ | **66/34/0** ✅ |
| 2 vers le jet parfait — 43/50/7 | 3/4/93 ❌ | 54/46/0 ⚠️ (−11) | **43/50/7** ✅ |
| 3 remontage, plancher — 15/50/35 | 7/30/63 ❌ | 26/50/24 ⚠️ (−11) | **14/50/36** ✅ (−1) |
| 4 création d'effet AU MIEUX — 32/50/18 | 2/3/95 ❌ | 66/34/0 ❌ (−34) | **34/50/16**  (−2) ✅ |
| 5 création d'effet AU PIRE — 1/0/99 | 1/0/99 ✅ | 1/0/99 ✅ | **1/0/99** ✅ |
| **Erreur quadratique totale** | **21 398** | **2 164** | **10** |

Préréglage retenu (`params_devblog_fit()`) :
```
rate_fm = 20.0 · facteur_diff = 0.8 · plafond_mstat = 1.0 · exposant_c = 1.5
exposant_mstat = 0.4 · exposant_taille = 0.2 · coef_exo = 0.25
coef_niveau_objet = 0.3 · penalite_par_over_exo = 0.7
bonus_mono_jet = 1.0 (non identifiable) · malus_ethere = 1.0 (non identifiable)
```

### Ce que le nouveau jeu d'ancres révèle sur la FORME de la formule

**(1) La forme StarLoco tient — mais uniquement parce que l'ancre 3 se lit autrement
qu'on ne le croyait.** C'est le résultat le plus important de cette révision.

Dans la famille StarLoco, avec `S = PWRg + PWRcarac` et `w` = poids de la rune :
```
P_SC = a / (√S + w)        P_SN = min( a / √S , 100 − P_SC , 50 )
⟹     P_SN / P_SC = 1 + w / √S          (tant que P_SN n'est pas au clamp de 50)
```
Ce rapport ne dépend **que** de `w/√S`. **Aucun paramètre calibrable ne peut le
modifier** : ni `rate_fm`, ni les exposants, ni les nouveaux facteurs (qui
multiplient `a` et se simplifient dans le rapport). Les ancres imposent donc :

| Ancre | Contrainte imposée sur `w/√S` |
|---|---|
| 2 — 43/50/7 | ≥ 7/43 = **0.16** (facile) |
| 4 — 32/50/18 | ≥ 18/32 = **0.56** (rune légère sur objet léger : atteignable) |
| 3 — 15/50/35 | ≥ 35/15 = **2.33** ← **très contraignant** |

Sur un « objet complexe haut-niveau » (`PWRg ≈ 700 à 1900`, donc `√S ≈ 27 à 44`),
`w ≥ 2.33·√S` exige une rune de **poids ≥ 63 à 100**. Seules **Ga Pa (100)**,
**Ga Pme (90)** et, marginalement, **Po (51)** qualifient. Avec une rune de
remontage ordinaire (Do = 20, Ra Fo = 10, Pa Sa = 9), l'ancre 3 est
**mathématiquement inatteignable** : le modèle plafonne vers 24/41/35, quel que
soit le jeu de paramètres — c'était exactement l'« écart irréductible de 10 points »
constaté dans la version précédente de ce document.

➡️ **La formule nous dit donc comment lire Ankama.** « Bonus **maximums** sur objets
complexes haut-niveau […] runes de puissance **suffisante** » ne signifie pas
« jets à leur maximum », mais **« les bonus les plus lourds » = PA / PM**, remontés
avec la rune correspondante. Sous cette lecture, l'ancre 3 devient reproductible à
1 point près, et l'écart irréductible **disparaît**. C'est une contrainte
structurelle qui a une valeur d'information : elle désambiguïse le texte source.

**(2) Le défaut de mise à l'échelle (U31) est confirmé et aggravé.** `a ∝ PWRmax`,
`b ∝ √PWRg` : StarLoco brut rend les gros objets *plus faciles*, contre le DevBlog.
L'ajustement choisit `exposant_taille = 0.2` (contre 0.4 auparavant) : il faut
**inverser** la tendance encore plus fortement une fois l'ancre 4 dans le jeu.

**(3) Le niveau de l'objet (U32) devient identifiable et NON nul.** L'ajustement
retient `coef_niveau_objet = 0.3` (≈ −30 % de chances à niveau 200 par rapport au
niveau 0), là où toutes les formules d'émulateur l'ignorent. Le DevBlog dit
« faiblement » : 0.3 est compatible avec ce qualificatif.

**(4) La création d'effet n'a pas besoin d'être un cas spécial.** Avec les mêmes
paramètres, `fm_sim.py` produit **34/50/16** (cible 32/50/18) sur une création d'effet facile
(objet léger niveau 20, jets bas, rune Fo) et **1/0/99** sur les **9** combinaisons
exo PA/PM/PO × 3 supports (léger nv60 → complexe nv200) — sans une seule ligne de
code du type `if exo: return 0.01`. Le 1 % communautaire tombe tout seul du plancher
`INV-1`, parce que les runes PA/PM/PO pèsent 100/90/51 et que la ligne n'est pas
naturelle. Le Monte-Carlo confirme : **996 exos PM / 100 000 = 0.996 %**,
IC95 [0.936 % ; 1.060 %], compatible avec la mesure réelle 1.11 % (T1).

### Ce qui reste irréductible

* **`malus_ethere` et `bonus_mono_jet` ne sont pas identifiables** sur ces 5 ancres :
  aucune des 5 situations d'Ankama ne met en jeu un objet éthéré ni un objet
  mono-jet. Le calibrateur les laisse à 1.0 (= neutre), ce qui est un artefact
  d'identifiabilité et **non** une réfutation — Ankama affirme **[OBS]** que ces
  facteurs existent. `params_devblog_fit_complet()` y injecte les valeurs
  suggérées `0.7` et `1.5`, marquées **[SUPP]** (le signe est [OBS], l'amplitude
  est inventée). **Priorité de mesure n°1** : une seule série sur un objet éthéré
  et une sur un objet mono-jet lèvent les deux d'un coup.
* **La valeur du plafond PAR OBJET** reste inconnue (§7.1). Le seul encadrement
  déductible du texte : elle laisse passer un PA seul (poids 100) mais pas PA+PM
  (poids 190) ⟹ **[100 ; 190)**.
* **Sur-ajustement** : 5 points, ~10 paramètres libres. Une erreur quadratique de 10
  n'est **pas** une preuve de validité prédictive ; c'est une preuve que la forme
  fonctionnelle est *assez riche* pour contenir les 5 ancres, ce qui n'était pas le
  cas avant (2 164 avec l'ancienne calibration, 21 398 pour StarLoco brut).
...
⚠️ **Précision d'accounting apportée par le DevBlog original** : la magie résiduelle
est créée **exactement quand on perd plus que prévu** (« lorsque ça arrive, de la
magie résiduelle est créée sur l'objet »), et elle « absorbera **en partie** les
échecs futurs » — *en partie*, pas totalement. `fm_sim.py` expose
`ModelParams.puits_absorption` (défaut 1.0 = absorption totale) et publie le bilan
complet dans `Outcome.absorbe_puits` :
```
puits' = max(0, puits − absorbé_pendant_la_perte + pertes_sur_les_stats − poids_rune)
```
Le terme `absorbé` évite de recréditer un puits déjà dépensé comme amortisseur ;
il est nul dès que le puits n'a rien encaissé, ce qui redonne la formule
communautaire habituelle.
...
⚠️ **Contrainte dure sur les sources de perte, DevBlog original [OBS]** :
> « Il est impossible de "puiser" dans les malus, c'est-à-dire les effets négatifs
> de l'objet, **à moins que ceux-ci ne soient overmaxés**, car ils joueraient
> souvent le rôle de puits sans fonds. »

Quel que soit l'ordre retenu, un **malus non overmaxé n'est jamais une source de
perte** : il est purement et simplement sauté. Seul un malus *overmaxé* — c'est-à-dire
FM'é **au-delà de son meilleur état naturel** (`value < jet_min`) — peut se dégrader,
et seulement jusqu'au retour à cet état naturel.
`ModelParams.puiser_malus_overmax_seulement`, défaut **True**.
...
| Les **malus s'aggravent** au lieu de baisser | **[CONS]** — c'est bien ce qui se passe en jeu | garder, **mais** seulement s'ils sont overmaxés (ci-dessous) |
| Un **malus non overmaxé** peut absorber de la perte | **FAUX**, contredit par le DevBlog **[OBS]** | **corriger** : le sauter entièrement (`puiser_malus_overmax_seulement`) |
| Les **malus** peuvent dépasser le malus maximum naturel | **FAUX** **[OBS]** : « les malus ne peuvent dépasser le malus maximum naturel » | **corriger** : plafonner (`plafonner_malus_au_max_naturel`) |
| Un **bonus** peut passer sous 0 | **FAUX** **[OBS]** : « les bonus d'un objet peuvent redescendre jusqu'à 0 au minimum » | **corriger** : plancher explicite à 0 |
...
PERTE(item, poids_a_perdre, stat_ciblée) :
    ...
    pour chaque ligne L dans cet ordre :
        ...
        si L.is_negative :                       # MALUS
            # « impossible de puiser dans les malus […] à moins qu'ils ne soient
            #   overmaxés, car ils joueraient le rôle de puits sans fonds »  [OBS]
            si NON overmaxé (L.value >= L.jet_min) : continue    # intouchable
            delta = max(1, floor(L.value * reste / 100))
            # « les malus ne peuvent dépasser le malus maximum naturel »     [OBS]
            L.value = min(L.value + delta, L.jet_min)   # retour, au plus, au naturel
            poids_perdu_total += delta * COEF_MALUS[L.stat] ; reste -= idem
            continue
        ...
        chute  = max(0, chute)                  # « les bonus peuvent redescendre
                                                #   jusqu'à 0 au minimum »       [OBS]
...
## 7.1 Les DEUX plafonds — le corpus n'en connaissait qu'un

⚠️ **Correction majeure.** Le DevBlog original énonce **deux** limites distinctes,
dans deux puces séparées. Tout le corpus communautaire les confondait sous un
unique « cap 101 ». **[OBS]**

### (a) Plafond PAR EFFET — celui qui vaut 101
> « Il est impossible de dépasser un jet naturel maximum si **la somme du power-rate
> non-naturel et du power-rate actuel de l'effet** dépasse une limite fixe. Il est
> par exemple impossible de dépasser **101 points de force** sur un objet dont le
...
```
C'est **de là** que sort `overmax = floor(101 / coef)`. Paramètre
`ModelParams.cap_effet_non_naturel = 101.0` (l'alias historique `cap_over` est
conservé). Toute tentative qui le franchirait est **impossible** (elle n'est même
pas proposée en jeu).

### (b) Plafond PAR OBJET — **valeur inconnue**
> « Il est impossible de dépasser **une limite fixe de puissance d'effets
> non-naturels, pour l'intégralité des objets** (cette limite empêche par exemple
> d'ajouter à la fois un PA et un PM aux objets qui ne possèdent ni l'un ni
> l'autre de base). »

C'est une contrainte **globale sur l'objet**, pas sur une ligne. Ankama **ne donne
pas sa valeur**. Seul encadrement déductible du seul exemple fourni :
```
PA seul  = poids 100        doit passer      →  cap ≥ 100
PA + PM  = poids 100 + 90   doit être refusé →  cap < 190
⟹  cap_objet_non_naturel ∈ [100 ; 190)                                  [SUPP]
```
Paramètre `ModelParams.cap_objet_non_naturel`, **défaut `None` = désactivé**, parce
que coder une valeur inventée serait pire que de ne rien coder. À mesurer : tenter
PA + PO (151), PM + PO (141), PA + 1 % dommages (100 + 15)… la première combinaison
refusée donne la borne supérieure.

⚠️ **Ne pas dériver `overmax` du plafond par objet** : `floor(101/coef)` vient
exclusivement du plafond (a).

⚠️ **Divergence 101 vs 110.** Un relais du DevBlog disait « cette limite admet **110** en plus
du PWRGmax » ; le texte original ne donne que l'exemple des **101 points de force**.
Toutes les sources modernes disent **101**. Les caps par stat observés en jeu
...
## 7.3 La CRÉATION D'EFFET — le « 1 % » est un PLANCHER, pas une constante

⚠️ **Correction majeure.** « L'exo, c'est 1 % » est **faux en général**. Le DevBlog
original donne **deux** ancres pour la création d'effet, pas une :

```
Probabilités MAXIMUM en création d'effet, pour un maître :   32 / 50 / 18
Probabilités MINIMUM en création d'effet, pour un maître :    1 /  0 / 99
```

La création d'effet couvre donc **tout l'intervalle 32/50/18 → 1/0/99** selon la
difficulté. Le 1 % que la communauté connaît est le **plancher**, atteint sur les
cas extrêmes : PA (poids de rune 100), PM (90), PO (51), sur un objet déjà chargé.
**Créer une ligne de Force sur un objet simple peut monter à ~32 %.**
Et le **succès neutre EXISTE en création d'effet** (jusqu'à 50 %) : il n'est nul
**qu'au plancher**.

| Fait | Statut |
|---|---|
| Création d'effet AU MIEUX = **32/50/18** | **[OBS]** DevBlog, ancre 4 — **jamais reprise par aucun guide en 15 ans** |
| Création d'effet AU PIRE = **1/0/99** | **[OBS]** DevBlog, ancre 5 |
| Un exo **PA / PM / PO** donne 1 % SC, 99 % EC, jamais de SN | **[CONS]** unanime — et c'est le **plancher**, atteint naturellement (rune de poids 100/90/51 + ligne non naturelle) |
| Mesure réelle : **111 exos PM / 10 000 tentatives = 1.11 %**, IC95 **[0.905 % ; 1.315 %]** | **[OBS]** |
| ➡️ **Ne pas coder « exo = 1 % »** : laisser le plancher `INV-1` produire le 1 %. `fm_sim.py` retombe sur 1/0/99 pour les 9 combinaisons PA/PM/PO × 3 supports, et sur 0.996 % en Monte-Carlo (100 000 tentatives), **sans câblage** | **[OBS]** |
| ~~« Avec puits, le triplet devient 1/22/77 »~~ | **SUPPRIMÉ** — ce triplet n'existe pas dans le texte d'Ankama |
| Maximum **1 PA + 1 PM + 1 PO** exotiques par objet (MàJ 2.3.4, 29/03/2011) | **[OBS]** — mais le DevBlog 1.27, lui, dit que le plafond PAR OBJET interdit PA **et** PM ensemble (§7.1b). Les deux règles ont coexisté à des époques différentes ; à trancher par version |
| Caps personnage : **12 PA / 6 PM / 9 PO** (équipements + consommables) | **[OBS]** |
| Hypothèse « P(SC exo) = 1 / poids de la rune » | **[SUPP] réfutée** — sinon le PO passerait 2× plus souvent que le PA |
| Pour les exos de **petit poids** (Do Per So, Invo…), le SC reste ~1 % mais le **SN augmente** | **[SUPP]** (kankhun) — **désormais cohérent** avec l'ancre 4 : un exo léger ne tombe pas au plancher |
| Un over est **beaucoup plus facile** qu'un exo (10 % observé sur une rés en over) | **[SUPP]** |

**Espérance et dispersion au plancher (loi géométrique, p = 1/100)** **[OBS]** :
```
E[X] = 100 tentatives · médiane ≈ 69 · σ ≈ 99.5
P(≥ 1 succès en n essais) = 1 − 0.99^n :   n=69 → 50 % · n=100 → 63 % · n=500 → 99.3 %
```
Un simulateur correct **doit** retomber sur ces chiffres **pour les exos PA/PM/PO**,
et **doit aussi** produire ~32 % sur une création d'effet facile. C'est le test
d'acceptation n°1 (T1), et c'est la porte de sortie qui garantit qu'on n'a pas
truqué le modèle en câblant le 1 %.

## 7.4 Dégressivité au-delà de 80 % du jet max — **[OBS]** DevBlog

> « Si le jet que l'on modifie est au-delà de 80 % du jet max naturel (plus
> précisément à 80 % de la fourchette du jet), la difficulté augmente. **Il s'agit
> d'un pallier**, la difficulté augmente brutalement à 80 % du jet. **Si le bonus a
> un jet fixe, ce facteur n'est pas pris en compte.** »
> (2ᵉ facteur de difficulté par ordre d'importance, texte primaire verbatim)

⚠️ **La clause « jet fixe » est inédite** : elle n'apparaît dans aucun guide.
Un bonus sans fourchette (PA, PM, PO, dommages fixes des objets qui n'ont qu'une
valeur possible) **échappe entièrement au palier des 80 %**. `fm_sim.py` :
`ModelParams.jet_fixe_ignore_palier`, défaut **True** ; `fill_ratio()` renvoie 0
pour un jet fixe non dépassé. Cela change sensiblement la difficulté d'un remontage
de PA naturel — précisément le scénario de l'ancre 3 (§3.7).
...
## 7.5 Le succès neutre — ce qu'en dit le texte primaire **[OBS]**

> « Lors d'une tentative de forgemagie, il est désormais possible d'obtenir un
> succès partiel (ou résultat neutre). […] le jet modifié augmente, et un autre jet
> diminue. **Si ce résultat n'est pas possible (objet qui ne dispose que d'un seul
> jet par exemple), rien ne se passe en cas de succès partiel.** »
>
> « Les chances d'obtenir un succès partiel sont **au maximum de 50 %**. Elles sont
> **proches de ce maximum dans la majorité des cas** : elles diminuent si la
> transformation est très facile, au profit du succès critique, ou si la
> transformation est très difficile, au profit de l'échec critique. »

Trois conséquences d'implémentation :
1. **SN sur objet mono-jet = NO-OP.** Pas de gain, pas de perte, pas de rune
   consommée du point de vue de l'état de l'objet, pas de mouvement de puits.
   `fm_sim.py` renvoie `Outcome(issue="SN", affichage="SN_NUL")` et ne touche à
   rien. Le corpus antérieur traitait ce cas comme un gain simple : **c'était faux**.
   Généralisation : le no-op s'applique dès qu'**aucune autre ligne ne peut
   diminuer** (objet vidé, ou dont les seules autres lignes sont des malus non
   overmaxés / des retraits PA-PM) — sauf s'il reste du puits pour encaisser.
2. Le SN est le résultat **modal** : la courbe P_SN(difficulté) est en cloche,
   saturée à 50 % sur une large plage centrale, et retombe aux deux extrémités.
   C'est exactement ce que produit `min(a/√S, 100 − P_SC, 50)` combiné au plancher.
3. Ce n'est **ni** un succès **ni** un échec : ne pas le compter dans un « taux de
   réussite » sans le dire.

## 7.6 Autres règles dures
...
> ⚠️ Ne pas confondre avec la « casse » d'un objet en FM : **elle n'existe pas** (§7.6).
...
| ~~U12~~ | ~~Le triplet « jet parfait »~~ | **43/50/7** | **TRANCHÉ** par le texte primaire (10/09/2026). Le 34/50/16 du relais Yin-Yang est FAUX | — | **clos** |
| ~~U13~~ | ~~`SN_AVEC_PUITS` (le 22 % de `1/22/77`)~~ | — | **SUPPRIMÉ** : le triplet 1/22/77 n'existe pas dans le texte d'Ankama | La calibration qui s'appuyait dessus était corrompue | **clos** |
...
| **U31** | `exposant_taille` — mise à l'échelle avec la taille de l'objet | **0.2** | **découvert ici** (§3.7). StarLoco (`e = 1`) rend les gros objets *plus faciles*, contrairement au DevBlog | Le facteur de difficulté **n°1** selon Ankama. Une erreur ici fausse tout | Protocole **A**, en faisant varier `PWRmax` (objets de niveaux/complexités très différents) à `f` constant |
| **U32** | `coef_niveau_objet` — terme dépendant du **niveau de l'objet** | **0.3** | 3ᵉ facteur cité par Ankama, **absent de toutes** les formules d'émulateur ; désormais identifié par l'ajustement (§3.7) | « faiblement » selon Ankama ; 0.3 = −30 % à niveau 200 | Protocole **A** sur 3 objets de même `PWRmax` mais de niveaux très différents |
| **U33** | `malus_ethere` (objets éthérés plus difficiles) | **1.0** (neutre) ; suggestion **0.7** | **[OBS]** pour l'existence et le signe, **[SUPP]** pour l'amplitude. **NON IDENTIFIABLE** sur les 5 ancres : aucune ne met en jeu un objet éthéré | Sous-estime la difficulté de tout le contenu éthéré | **Priorité n°1** : une seule série comparée éthéré / non-éthéré à état identique |
| **U34** | `bonus_mono_jet` (objets à un seul jet naturel plus faciles) | **1.0** (neutre) ; suggestion **1.5** | idem U33, **NON IDENTIFIABLE** sur les ancres | Fausse tous les petits objets (anneaux/amulettes bas niveau, Gelano…) | **Priorité n°1** : série sur un objet mono-jet |
| **U35** | `penalite_par_over_exo` (`a *= f ** n_over_exo`, **jet ciblé compté**) | **0.7** | **[OBS]** pour l'existence, la forme exponentielle est **[SUPP]** — Ankama dit seulement « plus il y en a, plus c'est dur » | Difficulté d'un 2ᵉ / 3ᵉ over sur un même objet | Protocole **A** sur un même objet à 0, 1, 2, 3 overs |
| **U36** | `jet_fixe_ignore_palier` | **True** | **[OBS]** verbatim (« si le bonus a un jet fixe, ce facteur n'est pas pris en compte ») | Change fortement le remontage des lignes PA/PM/PO **naturelles** | déjà tranché, ne pas calibrer |
| **U37** | `exclure_jet_cible_de_qualite` | **True** | **[OBS]** verbatim | Sur-estime la difficulté des objets à peu de lignes | déjà tranché, ne pas calibrer |
| **U38** | **Structure** : `P_SN/P_SC = 1 + w/√(PWRg+PWRcarac)`, **non modifiable par aucun paramètre** | — | conséquence de la forme StarLoco (§3.7) | Contraint la lecture même des ancres (cf. ancre 3) | Une mesure conjointe (P_SC, P_SN) sur un seul point suffit à réfuter/valider la forme |
...
| U19b | `puits_absorption` — « la magie résiduelle absorbera **EN PARTIE** les échecs futurs » | 1.0 (totale) | **[OBS]** que c'est partiel, **[SUPP]** pour la fraction | Sur-estime la protection du puits | Protocole **H** : comparer perte observée et poids de rune quand le puits est suffisant |
| U19c | Peut-on « puiser » dans un malus **non overmaxé** ? | **non** | **[OBS]** verbatim, tranché | Un malus jouerait le rôle de puits sans fond | **clos** |
...
| U30 | Cap over/exo PAR EFFET : 101 vs 110 | 101 (cohérence parfaite avec tous les caps par stat) | tenter une ligne à poids 102-110 en over : impossible ⇒ 101 |
| **U39** | Valeur du plafond **PAR OBJET** (`cap_objet_non_naturel`) | **INCONNUE**, encadrée dans **[100 ; 190)** par le seul exemple d'Ankama | Tester en jeu quelles combinaisons d'exos passent : PA+PO (151), PM+PO (141), PA+15 pts de poids (115)… La première refusée donne la borne haute. **Désactivé par défaut** (`None`) tant que la mesure n'est pas faite |
...
3. ~~**Le contenu exact du DevBlog 1.27**~~ — **RÉSOLU le 10/09/2026** : le texte
   intégral a été retrouvé sur `web.archive.org` et est recopié verbatim dans
   `sources/archive/DEVBLOG-ANKAMA-ORIGINAL.md`. C'était bien la piste la plus
   rentable : elle a tranché U12, supprimé U13, ouvert U33-U39 et **invalidé la
   calibration précédente**. Leçon méthodologique : **ne jamais calibrer sur un
   relais quand la source primaire peut encore être retrouvée**.
4. **La valeur du plafond PAR OBJET** (U39) : Ankama ne la donne pas, et elle n'est
   observable qu'en jeu, par essais successifs de combinaisons d'exos.
...
| `puits_avant` | **suivi par ton propre outil**, pas par le jeu (invisible) | Sépare les hypothèses U19b/U20/U21 |
...
en `f = 1.0` — qui doit retomber sur l'ancre 2, **43/50/7** (U12 est **clos**).
*Refaire l'expérience sur une ligne à **jet fixe** (PA, PM, PO naturel) : le palier
ne doit PAS apparaître (U36).*
...
**D — Création d'effet (U6, U8, U19b) — protocole REFONDU.**
Quatre séries de 500 tentatives : (a) exo PM sur objet **sans** puits, (b) exo PM sur objet
**avec** puits ≥ poids rune maintenu, (c) exo **d'une petite stat** (Force, Vitalité, Invo)
sur un objet **léger et bas niveau** aux jets bas, (d) le même exo léger sur un objet lourd.
Comparer les quatre `P_SC` **et** les quatre `P_SN`.
➡️ **Prédiction du modèle, à réfuter** : (a) et (b) donnent le **même** 1/0/99 — le puits ne
change **pas** le triplet (le `1/22/77` n'existe pas) ; (c) doit s'approcher de l'ancre 4
**32/50/18**, avec un SN **non nul** ; (d) doit être nettement plus bas que (c).
Si (c) reste à 1 %, l'ancre 4 est mal reconstruite et tout le §3.7 est à revoir.
*Un 2ᵉ exo sur le même objet (série e) teste U8 — mais c'est coûteux.*

**D2 — Les facteurs non identifiables (U33, U34) — PRIORITÉ N°1.**
Deux comparaisons appariées, 200 tentatives chacune :
(a) même template, une version **éthérée** vs une version normale, même état de jets ;
(b) un objet à **un seul jet naturel** vs un objet à 2 jets de poids total comparable.
Ce sont les deux seuls paramètres du modèle que les 5 ancres officielles ne
contraignent **pas du tout** : 400 tentatives suffisent à les lever.

**D3 — Le plafond PAR OBJET (U39) — gratuit, sans statistique.**
Tenter, sur un objet sans PA/PM/PO natif : PA seul (100), puis PA+PO (151), PM+PO (141),
PA+PM (190), PA + 15 points de poids d'une autre stat (115)… La **première combinaison
refusée par l'interface** donne la borne supérieure du plafond par objet, actuellement
seulement encadré dans **[100 ; 190)**.
...
* **vérifier qu'aucun malus non overmaxé ne bouge jamais** (U19c, [OBS] : « impossible de
  puiser dans les malus… à moins qu'ils ne soient overmaxés ») ;
* **vérifier qu'aucun malus ne dépasse son maximum naturel**, et qu'aucun bonus ne passe
  sous 0 ;
* mesurer la fraction de perte réellement encaissée par le puits quand il est suffisant
  → **U19b** (« en partie »).

**I — Le puits (U19b, U20, U21, U22). La question la plus ouverte du domaine.**
...
* si les **trois** taux sont identiques → **le puits n'influence pas les probabilités**.
  C'est désormais l'hypothèse par défaut du modèle : le DevBlog ne dit du puits que
  qu'il « absorbera en partie les échecs futurs », c'est-à-dire qu'il agit sur la
  **perte**, pas sur le tirage. Le triplet `1/22/77` qui suggérait le contraire est
  une invention d'un relais.
* si `P_SC` identique et `P_SN` supérieur en série 2 → le puits transforme des EC en SN
  (ancienne hypothèse par défaut, désormais **sans appui documentaire**).
...
T1b creation d'effet FACILE              -> P_SC ≈ 32 %, P_SN ≈ 50 %  (le 1 % n'est PAS cablé)
T1c exo PA/PM/PO x 3 supports            -> 9/9 au plancher 1/0/99, sans cablage
T2  exo PM avec puits                    -> triplet INCHANGE (1/0/99) : le 1/22/77 n'existe pas
...
T6  DEUX plafonds                        -> par effet : poids_non_naturel ≤ 101 ; par objet :
                                            PA seul OK, PA+PM refusé pour tout cap ∈ [100;190)
T7  bilan du puits                       -> puits' = max(0, puits − absorbé + pertes − poids_rune)
T8  les 5 ancres                         -> 66/34/0 · 43/50/7 · 15/50/35 · 32/50/18 · 1/0/99
T9  écart aux ancres                     -> ≤ 3 points sur les 5
T11 SN mono-jet                          -> no-op integral (aucun etat ne bouge)
T11 malus non overmaxé                   -> jamais source de perte ; malus plafonné au max naturel
T11 jet fixe                             -> palier des 80 % desactive
T11 jet ciblé                            -> COMPTE dans nb_over_exo, EXCLU de la qualité globale
```
...
3. ~~**DevBlog 1.27 original** sur `web.archive.org`~~ — **FAIT le 10/09/2026**, voir
   `sources/archive/DEVBLOG-ANKAMA-ORIGINAL.md` et §14.0. C'était bien la piste la plus
   rentable du corpus.
...
poids de ligne et d'objet · bilan du puits · plafond **par effet** 101 et caps par stat dérivés
de `101/coef` · `rune.add` fixe · **exo PA/PM/PO** = 1 % en SC, 0 % en SN (c'est le **plancher**,
pas une constante à câbler) · 1 PA + 1 PM + 1 PO max · disparition d'une ligne sous 1 ·
bonus plancher 0, malus plafonné au malus max naturel, malus non overmaxé intouchable ·
SN mono-jet = no-op · over prioritaire à la perte · pas de casse d'objet en FM ·
les bornes 1 % / 50 % / 66 % · les 5 ancres officielles du §3.1.

⚠️ **Ce que tu ne dois PAS coder en dur :** `if exo: return 0.01`. La création d'effet est un
**continuum 32/50/18 → 1/0/99** ; le 1 % doit sortir tout seul du plancher `INV-1`.
...
testable) → 3. `probabilite()` avec le préréglage **ajusté** du §3.7 (`params_devblog_fit_complet()`
pour simuler, `params_devblog_fit()` pour reproduire l'ajustement)
→ 4. tests T1-T11 → 5. collecte protocole **D3** (gratuit) puis **D2**, **B**, **F**
→ 6. recalibration → 7. le reste.

**Ce que ce document apporte de neuf par rapport au corpus de départ :**
* ⭐ la **source primaire** (`sources/archive/DEVBLOG-ANKAMA-ORIGINAL.md`), jamais citée
  intégralement par aucun guide en 15 ans, et les **7 corrections** qu'elle impose (§14.0) ;
* l'ancre inédite **32/50/18** (création d'effet au mieux) et la démonstration que
  « l'exo, c'est 1 % » est **faux en général** ;
* la suppression du faux triplet **1/22/77** et de la calibration corrompue qui en dépendait ;
* la séparation des **deux plafonds** (par effet = 101 [OBS] ; par objet = inconnu, [100;190)) ;
* trois facteurs de difficulté jamais modélisés — **éthéré**, **mono-jet**, **nombre
  d'over/exo (jet ciblé compté)** — plus les clauses **jet fixe** et **exclusion du jet ciblé
  de la qualité** ;
...
* **le résultat structurel** : `P_SN/P_SC = 1 + w/√(PWRg+PWRcarac)` n'est modifiable par
  aucun paramètre ; l'ancre 3 exige donc `w ≥ 2.33·√S`, ce qui **désambiguïse le texte
  d'Ankama** (« bonus maximums » = PA/PM, pas « jets au maximum ») et fait disparaître
  l'écart de 10 points qui semblait irréductible (§3.7) ;
* un préréglage qui reproduit **les 5 triplets officiels à ≤ 3 points** (erreur quadratique
  21 398 → 10) ;
...
# 14. ADDENDA — les deux sources d'archive, et ce qu'elles ont changé

## 14.0 ⭐ ADDENDUM PRINCIPAL — le DevBlog Ankama ORIGINAL (10/09/2026)

Le texte intégral du DevBlog « La nouvelle forgemagie » a été retrouvé sur la
Wayback Machine et recopié **verbatim** dans
[`sources/archive/DEVBLOG-ANKAMA-ORIGINAL.md`](sources/archive/DEVBLOG-ANKAMA-ORIGINAL.md)
(FR : `devblog.dofus.com/fr/billets/61-nouvelle-forgemagie.html` ;
DE identique : `/de/beitragen/11-neue-schmiedmagie.html`).

**C'est LA source primaire.** Tout ce qui précède dans ce document a été revu
contre elle. Les relais communautaires (Yin-Yang, Alterya, forgemagie.net/ExiTeD,
tutoriels Dofus-Touch) sont **rétrogradés au rang de sources secondaires, parfois
fautives** : aucun ne cite le texte intégralement, et plusieurs le citent faux.

### Les 7 corrections qu'il impose

| # | Correction | Où c'est traité |
|---|---|---|
| 1 | L'ancre « jet parfait » est **43/50/7** (le 34/50/16 de Yin-Yang est FAUX ; ExiTeD avait raison). L'intervalle `[34;43]` se referme sur 43 | §3.1, §14.2 |
| 2 | Le triplet **1/22/77 « exo avec puits » N'EXISTE PAS** — invention d'un relais. Toute calibration faite dessus était corrompue | §3.1, §3.6, §7.3, §11.1 (U13 supprimé) |
| 3 | **6ᵉ ancre inédite : création d'effet AU MIEUX = 32/50/18.** « L'exo, c'est 1 % » est FAUX en général : la création d'effet couvre 32/50/18 → 1/0/99, le SN y existe (jusqu'à 50 %) et n'est nul qu'au plancher | §3.1, §3.7, §7.3 |
| 4 | **Trois facteurs de difficulté jamais modélisés** : objets **éthérés** plus difficiles ; objets à **un seul jet naturel** plus faciles ; le **nombre d'over/exo** augmente la difficulté — et pour ce facteur-là **le jet en cours de modification EST compté**, alors qu'il est **exclu** de la qualité globale | §3.2, §3.6, §11.1 (U33-U35) |
| 5 | **DEUX plafonds distincts** : (a) par effet, `PWR non-naturel + PWR actuel ≤ 101` (d'où `overmax = floor(101/coef)`) ; (b) par objet, « limite fixe de puissance d'effets non-naturels », **valeur inconnue**, celle qui interdit PA+PM. Le corpus les confondait | §7.1, §11.3 (U39) |
| 6 | **Règles de perte** : bonus plancher **0** ; malus jamais au-delà du **malus maximum naturel** ; **impossible de puiser dans les malus non overmaxés** ; le puits absorbe « **en partie** » les échecs futurs | §5.1, §5.3, §6.1, §6.2, §11.2 |
| 7 | **SN sur objet mono-jet = rien ne se passe** (no-op), pas un gain | §7.5 |

### Deux clauses inédites bonus, trouvées dans le même texte
* « **Si le bonus a un jet fixe**, ce facteur [le palier des 80 %] n'est pas pris
  en compte. » → §7.4, `jet_fixe_ignore_palier`.
* « Il est plus difficile de forgemager un jet jusqu'à une valeur élevée **sans
  utiliser des runes puissantes**. » → la **règle du ×20** passe de **[CONS]** à
  **[OBS]** : Ankama la confirme, sans en donner le seuil (§7.2).

### Impact chiffré sur le modèle
Erreur quadratique sur les 5 ancres (détail en §3.7) :
**21 398** (StarLoco brut) → **2 164** (ancien préréglage, calibré sur les ancres
fausses) → **10** (nouveau préréglage). L'ancien préréglage se trompait de **34
points** sur la nouvelle ancre 4, qu'il n'avait aucune raison de connaître.

---

## 14.1 ADDENDUM SECONDAIRE — Wiki forgemagie.net (ExiTeD, 2015)

Le wiki de référence de la communauté FM francophone (forgemagie.net, dernière MAJ
28/12/2015) a été retrouvé via la Wayback Machine ; contenu dans
`sources/archive/forgemagie-net-wiki.md`. C'est la source dont dérivent la plupart
des guides FM français ultérieurs. **Source secondaire** : à lire *après* le
DevBlog, et à écarter partout où elle le contredit.

### Confirmations **[CONS]** renforcées, désormais adossées au primaire
* Formule du puits : `(Puits + PWR_Pertes) − PWR_Rune`. → §5.1
* Cap 101 sur over+exo, avec la déduction explicite `OVER = floor(101 / coef)`.
  ✅ confirmé par le DevBlog — mais c'est le plafond **PAR EFFET** seulement (§7.1).
* Règle du ×20 (difficile) / ×25 (quasi impossible sans puits). ✅ confirmée
  qualitativement par le DevBlog (« sans utiliser des runes puissantes »). → §7.2
* SN plafonné à 50 %, SC toujours ≥ 1 %. ✅ → §3.1
* « Le Jet en cours de modification n'est pas pris en compte dans le calcul du
  PWRG. » ✅ **confirmé mot pour mot** par le DevBlog. → §3.2, §14.3
* « Du niveau de l'objet : ce paramètre intervient dans une moindre mesure. »
  ✅ confirmé (« la difficulté augmente faiblement avec le niveau »). → §14.4

### ❌ Là où ExiTeD se trompe, ou dit moins que le primaire
* « Exo PA/PM/PO : probabilités **arbitraires** 1 % SC / 99 % EC, SN **impossible**,
  indépendantes de l'objet. » → **vrai pour PA/PM/PO, faux comme règle générale de
  la création d'effet** (ancre 4 = 32/50/18). Et ce n'est pas « arbitraire » : c'est
  le plancher de la même formule. → §7.3
* Il ne mentionne **ni** les objets éthérés, **ni** les objets mono-jet, **ni** le
  nombre d'over/exo, **ni** la clause du jet fixe, **ni** le plafond par objet,
  **ni** le no-op du SN mono-jet, **ni** l'interdiction de puiser dans les malus.
* ✅ En revanche il avait **raison** contre Yin-Yang sur l'ancre 2 (43/50/7).

## 14.2 ~~Correction 1 — l'ancre n°2 est contestée~~ → **TRANCHÉE : 43/50/7**

| Relais | SC | SN | EC | Verdict |
|---|---|---|---|---|
| ⭐ **DevBlog Ankama original** | **43** | **50** | **7** | **fait foi** |
| Wiki forgemagie.net (ExiTeD) | 43 | 50 | 7 | ✅ exact |
| Blog Guilde Yin-Yang | 34 | 50 | 16 | ❌ **faux** |
| Modèle calibré `fm_sim.py` | **43** | **50** | **7** | reproduit exactement |

L'intervalle `SC ∈ [34 ; 43]` que ce document gardait ouvert est **fermé sur 43**.
Ne plus traiter l'ancre 2 comme incertaine, et ne plus utiliser le 34/50/16 comme
cible de calibration : il tirait le modèle de 9 points vers le bas.

⚠️ **Leçon méthodologique.** Yin-Yang se trompait aussi sur le triplet 1/22/77
(qu'il a purement inventé). Un relais fautif sur un point l'est souvent sur
plusieurs : quand deux relais divergent, ne pas « couper la poire en deux » —
chercher la source primaire, ou marquer le point comme non calibrable.

## 14.3 Correction 2 — le jet en cours de modification est EXCLU du PWRG **[OBS]**
> DevBlog, verbatim : « **le jet en cours de modification n'est pas pris en compte
> dans le calcul de la qualité**, afin de limiter l'impact de ce facteur sur les
> objets bas-niveau ou avec un seul jet. »

✅ **Implémenté** : `Item.pwr_g(exclure=stat_ciblée)` alimente `mStat`
(`ModelParams.exclure_jet_cible_de_qualite`, défaut True). Les over et exos, eux,
comptent bien dans la qualité.

⚠️ **Subtilité que personne n'avait relevée** : le même jet ciblé est au contraire
**COMPTÉ** dans le facteur « nombre d'over/exo sur l'objet » (§3.2). Deux
traitements opposés du même jet dans la même formule — ce n'est pas une
contradiction du texte, c'est une asymétrie voulue.

## 14.4 Correction 3 — le NIVEAU DE L'OBJET **[OBS]**, désormais identifié
> DevBlog : « La difficulté augmente **faiblement** avec le niveau de l'objet. »

Facteur **absent de tous les émulateurs** (StarLoco, Ancestra, Nao). Implémenté :
`a *= max(0.05, 1 − coef_niveau_objet · niveau / 200)`. L'ajustement sur les 5
ancres corrigées retient **`coef_niveau_objet = 0.3`** (U32), soit ≈ −30 % de
chances à niveau 200 — compatible avec « faiblement ».

⚠️ La contradiction apparente avec « un exo PA ne dépend ni de l'objet, ni de ses
jets, ni de son niveau » reste levée de la même façon : l'exo PA est **au
plancher**, où plus aucun facteur ne joue.

## 14.5 Correction 4 — épargne d'une ligne lourde (hypothèse ExiTeD) **[SUPP]**
> DevBlog, verbatim : « **Le choix des bonus qui sont perdus est aléatoire. Si un
> bonus est trop puissant par rapport à ce qui doit être enlevé, il a une chance
> d'être épargné par la baisse, mais ce n'est pas systématique.** […] En
> compensation, lorsque ça arrive, de la magie résiduelle est créée sur l'objet,
> qui absorbera **en partie** les échecs futurs. »

L'hypothèse d'ExiTeD `P(la carac part quand même) = PWR_Rune × 100 / PWR_Jet` est
**cohérente avec le texte** mais reste **[SUPP]** : Ankama ne donne **aucun
chiffre**. À exposer en option dans `ModelParams`, jamais comme défaut.

Le texte primaire précise en revanche **quand** le puits est créé : **exactement
quand on perd plus que prévu**. Et il borne son effet : « en partie », pas
totalement (`ModelParams.puits_absorption`).
...
**Exception** : si l'objet a un over/exo **et** un puits, et que la rune utilisée
n'est pas différente de ladite carac, alors le **puits est prioritaire**.

C'est un **troisième ordre**, différent des deux du §5.3. Le DevBlog original
**ne donne aucun ordre** — il dit seulement que le choix est **aléatoire**, avec
une chance d'épargner les lignes trop lourdes. ➡️ Les trois ordres restent des
hypothèses **[SUPP]** ; le paramètre `ordre_pertes` à trois valeurs subsiste, et
la lecture la plus littérale du primaire est en réalité `"aleatoire"` + règle
d'épargne. **Ne pas présenter l'un des trois comme établi.**
